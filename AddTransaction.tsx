...
export default function AddTransaction() {
  ...
  return <div className="form-page">
    ...
    <div className="form-grid">
      ...
      <label>Category<div className="select-wrap"><select value={categoryId} onChange={e=>setCategoryId(e.target.value)}><option value="">Optional</option>{rootCategories.map(c=><option key={c.id} value={c.id}>{c.icon?`${c.icon} `:''}{c.name}</option>)}</select><ChevronDown size={16}/></div><button type="button" className="inline-create-link" onClick={()=>navigate('/categories')}>+ Manage categories</button></label>
