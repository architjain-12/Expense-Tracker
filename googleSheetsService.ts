...
export async function restoreFromGoogleSheets(){
  ...
  const txs=(data.transactions||[]) as any[]; const accounts=(data.accounts||[]) as any[]; const categories=(data.categories||[]) as any[]; const rules=(data.recurringRules||[]) as any[]; const budgets=(data.budgets||[]) as any[]; const investments=(data.investments||[]) as any[];
  await db.transaction('rw',[db.transactions,db.accounts,db.categories,db.recurringRules,db.budgets,db.investments],async()=>{if(txs.length)await db.transactions.bulkPut(txs);if(accounts.length)await db.accounts.bulkPut(accounts);if(categories.length)await db.categories.bulkPut(categories);if(rules.length)await db.recurringRules.bulkPut(rules);if(budgets.length)await db.budgets.bulkPut(budgets);if(investments.length)await db.investments.bulkPut(investments);});
...
export async function getGoogleSheetsRecoveryHash(): Promise<string | undefined> {
  const settings = await db.settings.get('app');
  if (!settings?.googleSheetsEnabled || !settings.googleSheetsEndpoint) return undefined;
  const url = new URL(settings.googleSheetsEndpoint);
  url.searchParams.set('action', 'GET_RECOVERY');
  if (settings.googleSheetsToken) url.searchParams.set('token', settings.googleSheetsToken);
  const response = await fetch(url.toString());
  const data = await response.json();
  if (!response.ok || !data.success) throw new Error(data.message || 'Could not read PIN recovery configuration.');
  return data.recoveryHash || undefined;
}

export async function saveGoogleSheetsRecoveryHash(recoveryHash: string): Promise<void> {
  const settings = await db.settings.get('app');
  if (!settings?.googleSheetsEnabled || !settings.googleSheetsEndpoint) throw new Error('Configure Google Sheets first.');
  const url = new URL(settings.googleSheetsEndpoint);
  url.searchParams.set('action', 'SET_RECOVERY');
  if (settings.googleSheetsToken) url.searchParams.set('token', settings.googleSheetsToken);
  const response = await fetch(url.toString(), { method: 'POST', headers: {'Content-Type':'text/plain;charset=utf-8'}, body: JSON.stringify({ action:'SET_RECOVERY', recoveryHash }) });
  const data = await response.json();
  if (!response.ok || !data.success) throw new Error(data.message || 'Could not save PIN recovery configuration.');
}
