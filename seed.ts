...
export async function ensureSeedData(): Promise<void> {
  ...
  if (!(await db.budgets.filter(b => !b.categoryId && b.period === 'MONTHLY' && b.startDate.slice(0,7) === currentMonthStart.slice(0,7)).count())) {
    const budgetNow = new Date().toISOString();
    await db.budgets.put({ id: `budget-overall-${currentMonthStart.slice(0,7)}`, amount: 40000, period: 'MONTHLY', startDate: currentMonthStart, createdAt: budgetNow, updatedAt: budgetNow });
  }
