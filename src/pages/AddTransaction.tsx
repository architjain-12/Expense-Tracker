...
import { ArrowLeft, ChevronDown, Clock3, Save, Plus } from 'lucide-react';
...
export default function AddTransaction() {
  ...
  const [quickCategoryOpen,setQuickCategoryOpen]=useState(false); const [quickName,setQuickName]=useState(''); const [quickParent,setQuickParent]=useState('');
  ...
  async function quickAddCategory(){const n=quickName.trim();if(!n)return;const now=new Date().toISOString();const item={id:newId('cat'),name:n,parentId:quickParent||undefined,active:true,sortOrder:categories.length*10,createdAt:now,updatedAt:now};await db.categories.put(item);await queueEntitySync('CATEGORY',item.id,'CREATE',item);if(quickParent){setCategoryId(quickParent);setSubcategoryId(item.id);}else setCategoryId(item.id);setQuickName('');setQuickParent('');setQuickCategoryOpen(false);}
  ...
  return <div className="form-page">
    ...
    <div className="form-grid">
      ...
      <label>Category<div className="select-with-action"><div className="select-wrap"><select value={categoryId} onChange={e=>{setCategoryId(e.target.value);setSubcategoryId('')}}><option value="">Optional</option>{rootCategories.map(c=><option key={c.id} value={c.id}>{c.icon?`${c.icon} `:''}{c.name}</option>)}</select><ChevronDown size={16}/></div><button type="button" className="inline-add-btn" onClick={()=>{setQuickParent('');setQuickCategoryOpen(true)}} title="Add category"><Plus size={13}/></button></div></label>
      {categoryId && <label>Subcategory<div className="select-with-action"><div className="select-wrap"><select value={subcategoryId} onChange={e=>setSubcategoryId(e.target.value)}><option value="">Optional</option>{subcategories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><ChevronDown size={16}/></div><button type="button" className="inline-add-btn" onClick={()=>{setQuickParent(categoryId);setQuickCategoryOpen(true)}} title="Add subcategory"><Plus size={13}/></button></div></label>}
    ...
    {quickCategoryOpen&&<div className="modal-backdrop"><div className="modal"><div className="panel-header"><div><h2>{quickParent?'Add subcategory':'Add category'}</h2><p>Optional quick setup without leaving the transaction.</p></div><button className="icon-btn" onClick={()=>setQuickCategoryOpen(false)}>×</button></div><div className="settings-grid-like"><label>Name<input autoFocus value={quickName} onChange={e=>setQuickName(e.target.value)} placeholder={quickParent?'Restaurants':'Utilities'}/></label>{!quickParent&&<label>Parent category<select value={quickParent} onChange={e=>setQuickParent(e.target.value)}><option value="">Top-level category</option>{categories.filter(c=>!c.parentId).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>}</div><div className="inline-actions"><button className="primary-btn" onClick={()=>void quickAddCategory()}>Add</button></div></div></div>}
