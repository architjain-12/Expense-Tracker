...
export default function Home(){
 ...
 const [estimatedDuesExpanded, setEstimatedDuesExpanded] = useState(false);
 ...
 <section className="panel budget-hero">
  <div
    ...
    onClick={() => setEstimatedDuesExpanded(v => !v)}
    style={{ cursor: "pointer"}}
  >
    <div>
      <h2>Estimated dues <span className="dues-chevron">
        {estimatedDuesExpanded ? "▲" : "▼"}
      </span></h2>
      <p>
        Recurring payments expected this month; they are not
        transactions until accepted.
      </p>
    </div>

    <div className="dues-summary">
      <span className="dues-badge">
        {formatCurrency(estimatedDues)}
      </span>
    </div>
  </div>

  <div className="budget-progress dues">
    <span
      style={{
        width: `${
          budget
            ? Math.min(100, (estimatedDues / budget) * 100)
            : 0
        }%`
      }}
    />
  ...
  <div className="budget-progress-labels"   onClick={() => setEstimatedDuesExpanded(v => !v)}
    style={{ cursor: "pointer"}}
  >
    <span>
      {estimatedDueItems.length} future expense
      {estimatedDueItems.length !== 1 ? "s" : ""}
    </span>
    <span className="dues-chevron">
        {estimatedDuesExpanded ? <span>Tap to hide</span> : <span>Tap to view</span>}
      </span>
  </div>
    ...
    <div className="estimated-dues-list">
      {estimatedDueItems.length === 0 ? (
        <div className="empty-inline">
          No estimated dues for this month.
        </div>
      ) : (
        estimatedDueItems.map(item => (
          <button
            key={item.id}
