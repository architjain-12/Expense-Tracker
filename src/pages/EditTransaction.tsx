...
export default function EditTransaction(){const {id}=useParams();const navigate=useNavigate();const transaction=useLiveQuery(()=>id?db.transactions.get(id):undefined,[id]);const accounts=useAccounts();const categories=useCategories();const [merchant,setMerchant]=useState('');const [notes,setNotes]=useState('');const [categoryId,setCategoryId]=useState('');const [subcategoryId,setSubcategoryId]=useState('');const [accountId,setAccountId]=useState('');const [dateTime,setDateTime]=useState(toDatetimeLocal(new Date().toISOString()));
 useEffect(()=>{if(!transaction)return;setMerchant(transaction.merchant||'');setNotes(transaction.notes||'');setCategoryId(transaction.categoryId||'');setSubcategoryId(transaction.subcategoryId||'');setAccountId(transaction.accountId);setDateTime(toDatetimeLocal(transaction.transactionDateTime));},[transaction]);
