...
export default function TransactionList({
  ...
  const groups = unique
    ...
    .reduce<Record<string, Transaction[]>>((acc, t) => {
      const key = new Date(t.transactionDateTime).toLocaleDateString("en-IN", {
        day: "numeric",
        month: "short",
      });
      (acc[key] ??= []).push(t);
        ...
        return (
          <section key={day} className="day-group">
            <div className="day-label">
              <span>{day}</span>
              <span>
                {dailyExpenses ? `Spent ${formatCurrency(dailyExpenses)}` : ""}
                {dailyIncome
                  ? `${dailyExpenses ? " · " : ""}Income ${formatCurrency(
                      dailyIncome
                    )}`
                  : ""}
              </span>
            ...
            {items.map((t) => {
              ...
              const sub = categoryMap.get(t.subcategoryId || "");
              ...
              return (
                ...
                >
                  <div className="transaction-main">
                    <strong>
                      {t.merchant || t.notes || sub?.name || cat?.name || "Transaction"}
                    </strong>
                    <span>
                      {sub?.name
                  ...
                  <div className="transaction-meta">
                    <time>
                      {new Date(t.transactionDateTime).toLocaleTimeString(
                        "en-IN",
