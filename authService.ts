...
/** Generate a human-friendly recovery code. Store only its hash locally/synced. */
...
export async function setPinRecoveryCode(code: string): Promise<void> {
  const normalized = code.trim().toUpperCase();
  if (normalized.length < 8) throw new Error('Recovery code is too short.');
  const current = await db.settings.get('app');
  if (!current) throw new Error('Application settings are not initialized.');
  await db.settings.put({ ...current, pinRecoveryHash: await hashPin(normalized) });
}

export async function resetPinWithRecovery(code: string, newPin: string, remoteHash?: string): Promise<boolean> {
  if (!/^\d{4,8}$/.test(newPin)) throw new Error('PIN must contain 4–8 digits.');
  const current = await db.settings.get('app');
  if (!current) return false;
  const expected = remoteHash || current.pinRecoveryHash;
  if (!expected || await hashPin(code.trim().toUpperCase()) !== expected) return false;
  await db.settings.put({ ...current, localPinHash: await hashPin(newPin), lockEnabled: true, lockMethod: current.passkeyCredentialId ? 'PASSKEY' : 'PIN' });
  return true;
}
...
export async function enablePasskey(): Promise<void> {
  const currentBefore = await db.settings.get('app');
  if (!currentBefore?.localPinHash) throw new Error('Set a PIN first. Face ID always keeps PIN as the fallback.');
  const credentialId =
